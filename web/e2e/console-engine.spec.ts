...
/**
 * The console's engines, and the one claim they rest on.
 *
 * A frame is a whole machine handed to something that steps it, so the
 * console can hand it to halfwave over HTTP or to a chip in the page
 * (games/localEngine.ts): rung 0, the switch-level solver, or rung 1, the
 * same solver with the recognised gates folded into counters. That is only
 * true if they agree byte for byte, and "they are the same engine, built
 * twice" is exactly the kind of claim this suite exists to check rather
 * than repeat: the first test boots Die Runner and runs one frame on rung 0
...
test("rung 1 in the page answers the same frame as the API, byte for byte", async ({ page }) => {
  test.slow();
  await page.setViewportSize(DESK);
  await open(page, GAMES);
  await chipServed(page);
  ...
  const has = await page.evaluate(async () => {
    const manifest = await (await fetch("/6502/chip/asset-manifest.json")).json();
    const wasm = await import(/* webpackIgnore: true */ "/6502/chip/" + manifest["pkg/v6502_wasm.js"]);
    return typeof wasm.HybridMachine === "function";
  });
  test.skip(!has, "this chip release has no rung 1 (HybridMachine) yet");

  const out = await page.evaluate(async ({ WATCH, FRAME, ORG }) => {
    const manifest = await (await fetch("/6502/chip/asset-manifest.json")).json();
    const wasm = await import(/* webpackIgnore: true */ "/6502/chip/" + manifest["pkg/v6502_wasm.js"]);
    await wasm.default();
    const chip = new wasm.HybridMachine();
    ...
    const apiBoot = await post("boot", { memory, watch: WATCH });
    // The crossing itself: the API's booted machine resumes on rung 1, the
    ...
    const differs = (a: Record<string, string | number>, b: Record<string, string | number>, keys2: string[]) =>
      keys2.filter((k) => JSON.stringify(a[k]) !== JSON.stringify(b[k]));
    const memoryDiff = (a: { pages: Record<string, string> }, b: { pages: Record<string, string> }) =>
      [...new Set([...Object.keys(a.pages), ...Object.keys(b.pages)])].filter((k) => a.pages[k] !== b.pages[k]);
    const CHIP = ["value", "pullup", "pulldown", "trans_on", "half_cycle"];
  ...
  expect(out.halfCycle, "the frame ran on rung 1").toBe(FRAME);
  expect(out.apiHalfCycle, "and over the API").toBe(FRAME);
  expect(out.pages, "the machine carries its memory").toBeGreaterThan(2);
  expect(out.unknownNodes, "every watched line is a node on this die").toEqual([]);
  expect(out.stepChip, "the chip state after one frame").toEqual([]);
