...
const L = {
  en: {
    ...
    noShelfH: "This account has not been given a shelf",
    noShelf: "Shelves are granted by hand. What goes on one is a dump of a cartridge you own, and we have no way to check that for everybody who signs in, so we do not offer it to everybody. Ask us and we will sort it out.",
    addH: "Add cartridges",
  ...
  ja: {
    ...
    noShelfH: "このアカウントにはまだ棚がない",
    noShelf: "棚は手で渡している。棚に載るのは自分が持っているカートリッジのダンプで、サインインした全員についてそれを確かめる手立てが私たちにはない。だから全員には出していない。声をかけてくれれば用意する。",
    addH: "カートリッジを追加する",
    add: ".nes ファイルを選ぶ",
