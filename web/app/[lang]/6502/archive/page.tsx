...
const PROSE = {
  ...
  ja: {
    ...
    intro: (
      <>
        visual6502.org は、ここにあるすべての土台のダイデータが生まれた場所だ。
        そこの wiki は 2021 年以降のある時点から HTTP 500 を返し続けている:
        背後の MediaWiki が壊れていて、フロントページは今も表示されるのに、
        記事はどれも読めない。これは、それが不可能になる前に Internet Archive
        から復元したものだ。
      </>
    ),
    notOurs: (
      <>
        <b>どれも私たちのものではない。</b>
        以下のページは捕捉された当時のまま保存され、独自のデザインを保っている。
        それこそが保存というものの要点だ。私たちの言葉はこのページに、彼らの
        言葉はあちらのページにある。素材は <b>CC BY-NC-SA 3.0</b>、Greg James
    ...
    wikiWhat: (
      <>
        レンダリングからではなく、wikitext から再構築した。ページの写しを
        保存することと、ページそのものを保存することの違いがそこにある:
        wikitext は新しい MediaWiki に再インポートできるが、HTML の
        スクリーンショットにはそれができない。
      </>
    ),
    wikiCount: "文書 129 本",
    ...
    galleryWhat: (
      <>
        ネットリストのトレース元になった写真を、再び閲覧可能にしたもの。
        アーカイブ自身の索引はチップ 40 個、写真 516 枚と言っている。ファイル数
        がそれより多いのは、各写真にサムネイルが付いているからだ。
      </>
    ),
    ...
    mirrorWhat: (
      <>
        捕捉されたままのミラー。ページが何を意味していたかではなく、実際に
        何と書いてあったかが問いになった時のために。
      </>
    ),
    mirrorCount: "捕捉時のまま",
    ...
    where1: (
      <>
        6502 サイトが配信しているのと同じディレクトリを、コピーせずにここへ
        エイリアスしている。一つの発行者、一組のバイト列、発行者自身が持つ
        二つ目のアドレス。NonCommercial な素材の保存物をこのリポジトリに
        コピーすれば再配布にあたる。それをうっかりやらないことについて{" "}
        <a href="https://github.com/tinymachines/public/blob/main/NOTICE.md">
          NOTICE.md
        </a>{" "}
        は明示的だ。
    ...
    where2: (
      <>
        内部リンクは旧サイトを根とする絶対パスだったので、この接頭辞の下では
        存在しないページを指していた。ファイル自体は編集してよいものでは
        ないから、バイト列が出て行く途中で書き換えている。
      </>
    ),
  },
} as const;
