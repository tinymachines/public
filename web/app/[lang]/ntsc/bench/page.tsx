...
const PROSE = {
  en: {
    what: (
      <>
        The pipeline from the repository, running in this page: NES dots
    ...
    slow: (notch: number, comb3: number, stamp: string) => (
      <>
        This is a lab instrument that now runs near the signal&rsquo;s
        own rate: at least {notch} frames/s on the notch filter and {comb3}
        on the three-line comb in one thread ({stamp}), after the speed
        work the repository&rsquo;s perf report records. Step advances the
        source one frame. Run is a real free-run: the source advances at
        its own exact rate against the wall clock, the page encodes what it
        can, and the drift counters print what a real display would have
        duplicated or dropped, exactly as the repository&rsquo;s pacing
        rules specify.
      </>
    ),
    patterns: (
    ...
    boarded: (commit: string, href: string, tag: string) => (
      <>
        ...
        {tag ? <> (tag {tag})</> : null} by our own build script, and the
        record carries the file hashes, so what this page runs is exactly
        what the repository&rsquo;s suite measured.
      </>
    ),
  },
  ja: {
    ...
    slow: (notch: number, comb3: number, stamp: string) => (
      <>
        これは実験器具で、いまや信号自身のレート近くで走る: 1 スレッドでノッチフィルタが毎秒 {notch} フレーム以上、3 ラインコムが {comb3}（{stamp}）。リポジトリの性能レポートが記録する高速化の後の数字だ。「1 フレーム進める」はソースを 1 フレーム進める。「走らせる」は本物のフリーランで、ソースは壁時計に対して自身の正確なレートで進み、ページはできる分だけエンコードし、実際のディスプレイなら重複・欠落させたはずの分をドリフトカウンタが表示する。リポジトリのペーシング規則そのままに。
      </>
    ),
    patterns: (
      <>
    ...
    boarded: (commit: string, href: string, tag: string) => (
      <>
        バンドルはうちのビルドスクリプトが{" "}
        <a data-address href={href}>{commit}</a>
        {tag ? <>（タグ {tag}）</> : null} からビルドし、記録がファイルのハッシュを持つ。このページが走らせるものは、リポジトリのスイートが実測したものそのままだ。
      </>
    ),
  },
} as const;
