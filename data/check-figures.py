#!/usr/bin/env python3
"""The chip's figures, wherever prose states them, must be the recorded ones.

START-HERE.md: "no number is typed into a page: every figure is a slot filled
from a published file, and a harness re-derives them and scans the prose for
stray digits." The front page reads data/chip.json directly, so it cannot
drift. The documentation tree cannot: it is markdown that a non-developer
edits, and seven sentences across six files state the wire and switch counts
in passing.

So this is the scan. It finds four-digit numbers used as a chip figure, by
looking for the words that make them one, and fails when any of them is not a
value in chip.json.

    python3 data/check-figures.py

What it catches is the case that actually happens: chip.json is updated after
verify-chip.py finds a disagreement, and six prose files quietly keep the old
number. Prose is the part of a site most likely to go quietly wrong, because
it is written once against what was true that afternoon and nothing checks it
afterwards.
"""

from __future__ import annotations

import json
import re
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
CHIP = ROOT / "data" / "chip.json"

# A four-digit number counts as a chip figure when one of these words follows
# it closely. Deliberately narrow: "6502" is a chip name and "2026" is a year,
# and neither should be dragged into this.
CONTEXT = r"(?:wires?|switches|switch|nodes?|transistors?|byte)"
# A register address ($4015, $2002) is four digits after a dollar sign and
# is never a count of anything; the console arc's reports say "the $4015
# status byte" and the first pull of them tripped here. The narrowing is on
# the sigil, not on the number, so "4015 bytes" would still be checked.
# Not preceded by a $ (a hex address) nor by a hyphen (a part number such
# as an LDO's MCP1700-3302, found in the bench's parts list).
FIGURE = re.compile(rf"(?<![\$\-])\b(\d{{4}})\b[\s\-]*(?:\S+\s+){{0,2}}?{CONTEXT}\b", re.I)

# The atlas's own counts, which are two and three digits and were therefore
# invisible to the rule above. That blind spot is not hypothetical: the atlas
# route says 138 containers with 122 nodes in more than one, and this tree's
# own page said 135 and 88 from 2026-08 until 2026-09-20, when translating it
# turned them up. Narrow in the same way: a number counts only when one of
# these words is next to it.
ATLAS_CONTEXT = r"(?:containers?|groups)"
# These two run only on documents that are about the atlas, which the file
# says by naming it. "Groups" is an ordinary word elsewhere: the NES reports
# have "the two groups' numerators, 95.88" and "groups the two rules decide
# differently, is 121", and holding those to the chip's atlas would be a scan
# that fails on prose, which trains people to silence it.
#
# Both ways round, because the sentence that drifted puts the noun first:
# "The containers are the same derivations unfiltered: 138 of them". A rule
# that only looked forward passed it, which is how the first version of this
# scan missed the very number it was written for.
ATLAS = re.compile(rf"(?<![\$\-])\b(\d{{2,4}})\b[\s\-]*(?:\S+\s+){{0,2}}?{ATLAS_CONTEXT}\b", re.I)
ATLAS_BACK = re.compile(rf"{ATLAS_CONTEXT}\b(?:[^.\n]{{0,60}}?)(?<![\$\-\d])\b(\d{{2,4}})\b", re.I)

# Chip designations, which are four digits and are never a count of anything.
# The comment above already claimed 6502 would not be dragged in, and the
# narrowing above was not enough to keep that promise: the Die Runner page says
# a module was copied "byte for byte from tinymachines/6502", and `6502` two
# words before `byte` is exactly the shape this looks for. It fired on the
# chip's own name in a sentence about a file copy.
#
# This is a designation list rather than an exception list, and the difference
# matters: an exception list is where failures go to be silenced, so it grows
# whenever the check is inconvenient.
#
# The first version of this skipped a designation outright, which bought
# silence at the price of a real blind spot: a page stating "6502 nodes" would
# have passed, and the die has 1725. So the skip is conditional on DISTANCE.
# "6502 nodes" is a claim about the chip and is still checked; "6502`, apart
# from one line ... byte" has words in between and is prose about a part
# number. ADJACENT matches only the first shape.
DESIGNATIONS = {6502, 6800}
ADJACENT = re.compile(rf"^\d{{4}}[\s\-]*{CONTEXT}$", re.I)

SEARCH = [ROOT / "docs", ROOT / "web" / "app", ROOT / "style"]

# A document that says it is generated is exempt, narrowly. This scan exists
# for prose typed by hand against what was true that afternoon; the documents
# pulled from the 6502 tree carry "Generated by `tools/..." in their opening
# lines because their generator derived every number and refuses to write a
# file that fails its own checks. Scanning them here would hold their derived
# counts to OUR chip.json, which records different figures (this die has 1725
# wires in one counting and 3308 in the atlas's), and a scan that fails on a
# measurement is a scan training people to silence it.
def is_generated(text: str) -> bool:
    return "Generated by `tools/" in text[:600]


def shadows_a_generated_doc(path: Path) -> bool:
    """docs/ja/x.md is the same document as docs/x.md, in Japanese.

    The sentence that exempts a generated page is itself translated there, so
    the exemption above does not see it, and the shadow of a derived table got
    held to figures the generator never used. The shadow follows its English.
    """
    try:
        rel = path.relative_to(ROOT / "docs")
    except ValueError:
        return False
    if rel.parts[0] != "ja":
        return False
    english = ROOT / "docs" / Path(*rel.parts[1:])
    return english.is_file() and is_generated(english.read_text(errors="replace"))
SUFFIXES = {".md", ".mdx", ".tsx", ".ts", ".html"}


def main() -> int:
    chip = json.loads(CHIP.read_text())
    # Every recorded integer figure, not a hand-kept subset. The atlas
    # partition covers 1547 of the die's 1725 nodes, and the docs state both.
    # Recording 1547 as a figure with its own derivation is the right answer;
    # adding it to an exception list would have been the wrong one, because an
    # exception list is where failures go to be silenced.
    allowed = {v for v in chip.values() if isinstance(v, int)}
    print(f"chip.json figures: {sorted(allowed)} (measured {chip['measured_on']})")

    bad, seen = 0, 0
    for base in SEARCH:
        if not base.exists():
            continue
        for path in sorted(base.rglob("*")):
            if path.suffix not in SUFFIXES or not path.is_file():
                continue
            text = path.read_text(errors="replace")
            if is_generated(text) or shadows_a_generated_doc(path):
                continue
            about_atlas = "atlas" in text.lower()
            for n, line in enumerate(text.splitlines(), 1):
                matches = list(FIGURE.finditer(line))
                if about_atlas:
                    matches += list(ATLAS.finditer(line)) + list(ATLAS_BACK.finditer(line))
                for m in matches:
                    value = int(m.group(1))
                    if value in DESIGNATIONS and not ADJACENT.match(m.group(0)):
                        continue          # a part number in prose, not a count
                    seen += 1
                    if value not in allowed:
                        rel = path.relative_to(ROOT)
                        print(f"{rel}:{n}: states {value} as a chip figure, "
                              f"which is not in chip.json {sorted(allowed)}.")
                        print(f"    {line.strip()[:96]}")
                        bad += 1

    if seen < 5:
        print(f"check-figures: only {seen} figures found in the tree. These are "
              "stated in several places, so the scan is wrong and this check "
              "would pass on nothing.")
        return 2
    if bad:
        print(f"\ncheck-figures: {bad} stray figure(s) across {seen} found.")
        return 1
    print(f"check-figures: {seen} stated figures, all agree with chip.json")
    return 0


if __name__ == "__main__":
    sys.exit(main())
