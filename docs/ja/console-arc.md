...
この弧は、それぞれ単独で証明済みだった部品からコンソール全体を組み上げることを目指した。スイッチレベルの 6502 とそれに対して証明した高速コア、スイッチレベルの 2A03 と 2C02、波形のレベルで模擬した NTSC 信号経路、そして NES-001 基板の書き下ろしの糊。弧を走らせたスケッチは[ノートの最初のページ](/ja/docs/nes/sketch)にあり、以下の各ページが述べる数値は[コンソールのページ](/ja/nes)にあって、リポジトリ自身の走行から記録されたものだ。ここでは何も打ち直さない。
