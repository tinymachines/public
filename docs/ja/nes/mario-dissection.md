...
毎回ずれていたので、その行は最大 0.7 高く出ていた。「実機でやったこと」を
参照。) だからブランクは DMA、VRAM バッファ、そして音とパッドのルーチンに
...
ブリッジの受領で数を消すようにすると、押下はリセット無しで通る。1 ラッチの
ものと 3 ラッチのものだ (走行 193915、194105、194257)。ポールはタイトルの
247 行と 249 行にある。その顛末は `open-items.md` にある。模型の記録から読んだ
