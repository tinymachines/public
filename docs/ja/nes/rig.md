...
今日は入っていないもの: 二つのボードの検査が地図に対する判定を出すのは `--baseline` のときだけだ。暗いフレームや、ボードが一つも見つからない読みは、どの実行でもやはり失敗する。しかし半穴の比較が報告されるのは基準の実行のときに限られ、これは実験台についての規則ではなく `tools/rig-check.py` の穴である。
